...
export default function Home() {
  ...
  return <main className={`app-shell ${active.bg}`}>
    <aside className="sidebar"><div className="brand"><span className="brand-dot" /> LOTUS <b>99</b></div><div className="case-label">{t("案件档案 / 03:00")}</div><nav className="chapter-list" aria-label={t("章节")}>{chapters.map((item, index) => <button key={item.title} disabled={index !== chapter} className={index === chapter ? "chapter active" : "chapter locked"}><span>0{index + 1}</span><strong>{item.title}</strong><small>{index === chapter ? item.sub : t("由故事状态解锁")}</small></button>)}</nav></aside>
